...
import { getFavoritesLocal, getFavoriteTimestamps, toggleFavoriteLocal, syncFavoriteToSupabase } from "@/helpers/favorites";
...
export default function FavoritesClient() {
	...
	useEffect(() => {
		...
		async function load() {
			...
			try {
				...
				const res = await fetch(`/api/favorites/songs?device_id=${encodeURIComponent(deviceId)}`, { cache: "no-store" });
				if (!cancelled) {
					if (!res.ok) {
						setSongs(null);
					} else {
	...
	const handleDelete = async (customSongId: string) => {
		...
		setTs(getFavoriteTimestamps());
		setSongs((prev) => (prev ? prev.filter((s) => s.custom_song_id !== customSongId) : prev));
	...
	if (!sortedSongs) {
		// Fallback: render minimal info based on custom IDs
		return (
					...
					<div key={song.id} className="flex flex-col">
						<div className="flex items-center justify-between p-2">
							<div className="flex space-x-3">
								<SongCover url={song.cover_url || "https://ams1.reliastream.com/static/scarna00/covers/nocover.png"} artist={song.artist?.name || ""} type="hero" />
								<div className="flex flex-col">
									<div className="flex items-center">
										<div className="mr-2 hidden sm:inline-block md:inline-block lg:hidden xl:inline-block">❤</div>
										<div>
							...
							<div className="flex items-center gap-2">
								{addedAt && (
									<div className={`py-2 px-4 rounded-full ${i % 2 !== 0 ? "bg-tertiaryShade_1" : "bg-secondaryShade_1"}`}>
										<p className={`text-sm ${i % 2 !== 0 ? "text-tertiary" : "text-secondary"}`}>
